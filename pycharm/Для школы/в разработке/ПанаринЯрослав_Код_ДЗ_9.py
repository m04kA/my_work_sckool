...
fraza = 'привет'#input('Введите вашу фразу: ').lower().split()
alfovit = alf(alfovit)
fraza = frz(fraza)
fraza = preobr(fraza, alfovit)
print(f'Фраза, преобразованная в цифры: {fraza}')
fraza = with_dls(fraza, my_key[1])
print(f'Фраза, преобразованная по условию Кирилл Сергеевича: {fraza}')
dvoi_fraz = bin_text(fraza)
print(f'Перевод фразы в 2-ный код: {dvoi_fraz}')
dvoi_fraz = razbitie_na_dvoich(dvoi_fraz)
print(f'Разбитие по битам: {dvoi_fraz}')
dvoi_fraz = desitic_cisl(dvoi_fraz)
print(f'Преобразованная фраза в 10-ной СС: {dvoi_fraz}')
