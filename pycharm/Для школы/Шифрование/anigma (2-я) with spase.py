fraza_a = input('Введите вашу фразу: ')
fraza = fraza_a.split()
fraza= ''.join(fraza)
...
def surplus(g, j, k, row, col, fraza, tabl):
    g = 0
    for k in range(row):        #Переходим на следующую строчку  
        if k%2 ==0:    #Чётность строчки
            for j in range(col):  # Проверяем строчку
                if g > (len(fraza) - 1):
...
print_tabl(tabl)
#Чистка таблицы
surplus(g, j, k, row, col, fraza, tabl)
